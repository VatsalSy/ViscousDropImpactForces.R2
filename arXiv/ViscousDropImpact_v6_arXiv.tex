...
\shorttitle{\VS{The role of viscosity on drop impact forces}}
\shortauthor{V. Sanjay, B. Zhang, C. Lv, and D. Lohse}

%\title{Inertia \& viscosity dictate drop impact forces}
\title{\VS{The role of viscosity on drop impact forces}}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{Figure1_summary_v5.pdf}
...
Naturally, even the process of a Newtonian liquid drop impacting a rigid substrate is governed by a plethora of control parameters, including but not limited to the drop's density $\rho_d$, diameter $D_0$, velocity $V_0$, dynamic viscosity $\eta_d$, surface tension $\gamma$, \VS{and acceleration due to gravity $g$} (figure~\ref{fig:schematic}a). 
...
\noindent \VS{Additionally, the Bond number }
...
\noindent \VS{compares gravity to inertial forces and is needed to uniquely define the non-dimensional problem.}  
